...
func CreateGame(creator int, roomType string,
	mineNum int, boardSize int, timeLimit int) (*mongo.InsertOneResult, error) {
	collection := DB.Collection(gameCollection)
	newUuid := uuid.New().String()
	roomId := newUuid[len(newUuid)-6:]
	newGame := Game{RoomId: roomId, User1: creator, RoomType: roomType, MineNum: mineNum,
